...
int lca(int u,int v) // O( log(n) )
{
  ...
  // Get path max from u up to v (where u is deeper or equal)
ll jump_path(int u,int v) // O( log(n) )
{
  ...
  while(depth[u] >= depth[v])
  {
    ...
    if(depth[h] <= depth[v])
    {
      ans = max(ans,cal(tim[v],tim[u]+1)); // Add segment of the last heavy chain
      break;
    }
    else 
    {
...
int main()
{
    ...
    for(int i = 1;i<=n;i++)t[tim[i]+n] = a[i]; // (!) update at tim[i], not i
    build();
    ...
    while(q--)
    {
      ...
      if(i == 1)
      {
        int u,w;
        cin>>u>>w;
        up(tim[u],w); // (!) update at tim[u], not u
      }
      else
      {
        int u,v;
