...
struct CHT_General : multiset<Line, less<void>>
{
    ...
    void add(Line A) // O( log(n) ) amortized
    {
        ...
        while(!check(a,b)) b = erase(b); 
          ...
          // ___ infront of the new line
        bool forced = 1; // we must check the new line's validity AND one of the lines infront
        while(a != begin())
        {
            b = a;
            a = prev(a);
...
int main()
{
    ...
    {
        if(i == 1)  dp[i] = 0; 
        else  dp[i] = container.cal(a[i]) + a[i] * a[i] + pre[i-1];
          // Add new line for future DP queries
        container.add(Line( a[i] * -2, a[i] * a[i] - pre[i] + dp[i] ));
    }
