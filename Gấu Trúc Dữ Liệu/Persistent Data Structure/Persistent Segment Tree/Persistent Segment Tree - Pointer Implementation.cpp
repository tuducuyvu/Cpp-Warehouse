// Persistent Segment tree - Pointer Imple
/* documents
...
struct per_segtree 
{
    struct Node
    {
        ...
        Node(ll tmp) : val(tmp) {} // Construct leaf node with value
        Node(ll tmp,Node *L, Node *R) : val(tmp), l(L), r(R) {} 
    };
    
    vector<Node*>root; // the root of all versions
    
    int treeL,treeR; // version counters, tree boundaries
    ...
    Node *build(int tl,int tr) // O( n * log(n) )
    {
        ...
        if(tl == tr) return new Node(a[tl]); // If leaf, create node with array value
        else
        {
            int mid = (tl + tr)/2;
    ...
    Node *update_recur(Node *id,int tl,int tr,int pos,int val) // O( log(n) )
    {
        ...
        if(tl == tr) return new Node(val);            // If leaf, create new node with updated value
        
        int mid = (tl + tr)/2;
        
        Node *L = update_recur(id->l,tl,mid,pos,val);     // Update left child if needed
        Node *R = update_recur(id->r,mid+1,tr,pos,val);   // Update right child if needed
        
        return new Node(L->val + R->val, L, R);           // Create new node combining updated children
    }
    ...
    ll cal_recur(Node *id,int l,int r,int tl,int tr) // O( log(n) )
    {
          // Recursively query sum in range [l, r] for subtree [tl, tr]
        if(l > r || tl > tr || tl > r || tr < l)return 0;   // No overlap
        if(tl >= l && tr <= r)return id->val;               // Complete overlap, return sum
        int mid = (tl + tr)/2;
          // Partial overlap, sum left and right children
        return cal_recur(id->l,l,r,tl,mid) + cal_recur(id->r,l,r,mid+1,tr);
    }
